...
**【添加的发券商户号】**新增到活动中的发券商户号列表

 add_request_no 　必填   string(128)
...
```
1curl -X POST \
2  https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/126002309/merchants/add \
...
8      "100123456"
9    ],
10    "add_request_no" : "100002322019090134234sfdf"
11  }'
12
```
...
23 * 新增活动发券商户号
24 */
25public class AddActivityMerchant {
26  private static String HOST = "https://api.mch.weixin.qq.com";
27  private static String METHOD = "POST";
28  private static String PATH = "/v3/marketing/paygiftactivity/activities/{activity_id}/merchants/add";
29
30  public static void main(String[] args) {
31    // TODO: 请准备商户开发必要参数，参考：https://pay.weixin.qq.com/doc/v3/merchant/4013070756
32    AddActivityMerchant client = new AddActivityMerchant(
...
38    );
39
40    AddActivityMerchantRequest request = new AddActivityMerchantRequest();
41    request.activityId = "126002309";
42    request.merchantIdList = new ArrayList<>();
43    {
44      request.merchantIdList.add("100123456");
45    };
46    request.addRequestNo = "100002322019090134234sfdf";
47    try {
48      AddActivityMerchantResponse response = client.run(request);
49        // TODO: 请求成功，继续业务逻辑
...
55  }
56
57  public AddActivityMerchantResponse run(AddActivityMerchantRequest request) {
58    String uri = PATH;
...
91			return nil, err
92		}
93		response := &AddActivityMerchantResponse{}
...
106}
107
108type AddActivityMerchantRequest struct {
109	ActivityId     *string  `json:"activity_id,omitempty"`
110	MerchantIdList []string `json:"merchant_id_list,omitempty"`
111	AddRequestNo   *string  `json:"add_request_no,omitempty"`
112}
113
114func (o *AddActivityMerchantRequest) MarshalJSON() ([]byte, error) {
115	type Alias AddActivityMerchantRequest
116	a := &struct {
...
125}
126
127type AddActivityMerchantResponse struct {
128	ActivityId            *string                      `json:"activity_id,omitempty"`
129	InvalidMerchantIdList []InvalidParticipateMerchant `json:"invalid_merchant_id_list,omitempty"`
130	AddTime               *string                      `json:"add_time,omitempty"`
131}
132
133type InvalidParticipateMerchant struct {
134	Mchid         *string `json:"mchid,omitempty"`
135	InvalidReason *string `json:"invalid_reason,omitempty"`
136}
137
```
...
 activity_id 　选填   string

**【活动Id】**活动Id

 invalid_merchant_id_list 　选填   array[object]

**【校验失败的发券商户号】**未通过规则校验的发券商户号列表

| ![](https://gtimg.wechatpay.cn/resource/xres/img/202409/2a1a0741836e71dcb14320f2fcac3a6e_32x32.svg) | 属性 |
| --- | --- |
|  | mchid 　选填   string(15)【商户Id】商户Id  invalid_reason 　选填   string【无效原因】活动参与商户校验失败的原因 |
...
```
1{
2  "activity_id" : "1001234567",
3  "invalid_merchant_id_list" : [
4    {
5      "mchid" : "1001234567",
6      "invalid_reason" : "商户与活动创建方不具备父子关系"
7    }
8  ],
9  "add_time" : "2015-05-20T13:29:35.120+08:00"
10}
11
```
 
...
| 400 | INVALID_REQUEST | 发券商户号校验失败，请核实是否满足同品牌等规则 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
| 400 | INVALID_REQUEST | 发券商户号非奖品的同品牌商户号 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
| 400 | INVALID_REQUEST | 券归属商户或发券商户非同品牌 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
| 400 | INVALID_REQUEST | 创建方商户与发券商户非同品牌 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
| 400 | MCH_NOT_EXISTS | 商户号不存在 | 请确认发券商户号信息是否有误 |
| 400 | PARAM_ERROR | 创建活动参数错误，请联系技术支持 | 根据错误提示，传入正确参数 |
| 400 | PARAM_ERROR | 活动曝光商户的同品牌商户中需要至少存在一个与创建方商户具备父子关系 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
...
| 403 | RULE_LIMIT | 所有的曝光商户号均校验失败，请再次检查 | 发券商户号校验失败，请核实是否满足同品牌等规则 |
