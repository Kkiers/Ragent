...
1. 在Apache配置文件中取消UserDir，可能遇到的两种情况：（1）注释掉以下三行，然后重启Apache：                                                                                              ![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/d3e36f8bfe03cdcfa2a458e23e88f4ac.png)  （2）如果配置文件中没有上图中所示的三行，则添加（或修改）一行配置：UserDir disable，然后重启Apache
...
1. 对于Apache2，注释掉Apache配置文件中的目录配置中的以下两行：Alias /icons/ "/usr/share/apache2/icons/" AliasMatch ^/manual(?:/(?:de|en|es|fr|ja|ko|ru))?(/.*)?$ "/usr/share/apache2/manual$1"
...
检测方法：在浏览器里输入http://ip/xxx.xxx?a=alert(0) ，若出现如下弹框，则说明存在XSS漏洞。
