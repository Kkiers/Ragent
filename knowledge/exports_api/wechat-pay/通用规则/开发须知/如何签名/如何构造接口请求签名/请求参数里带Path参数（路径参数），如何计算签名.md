# 请求参数里带Path参数（路径参数），如何计算签名

# 请求参数里带Path参数（路径参数），如何计算签名

路径：通用规则/开发须知/如何签名/如何构造接口请求签名/请求参数里带Path参数（路径参数），如何计算签名

## 1、怎么判断当前请求的API是否属于请求参数里带Path参数？

你可以点击查看一个API接口说明文档，如[查询单笔退款（通过商户退款单号）](https://pay.weixin.qq.com/doc/v3/merchant/4012791863)，如下图，可以看到该接口请求参数部分带有out_refund_no这个path参数，因为不同的参数类型构造请求签名略有不同，

为了方便大家理解，我们将使用一对测试的商户API证书公私钥，通过命令行形式调用"查询单笔退款（通过商户退款单号）"接口，一步一步介绍请求参数里带Path参数（路径参数），如何计算签名。强烈建议你使用测试的商户API证书公私钥，按照本文实际操作一下，如果算出来的签名和文档的一致则说明计算无误。实际业务中请替换为你的真实的参数。
 
![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/328106c749fd4c4121d6d9e1e9c7146a.png)  
 
## 2. 如何构造请求签名

### 1. 获取商户API证书

构造请求签名需要使用商户API证书，可以参考[申请商户API证书](https://pay.weixin.qq.com/doc/v3/merchant/4012072428)

此处我们已经拿到了商户API证书，并且将对应的商户API证书私钥apiclient_key.pem，保存在了本地
 
商户号：1900007291

商户API证书序列号：408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB

商户API证书私钥：你需要将以下文件保存为pem格式，为了避免大家和自己的真实的公私钥混淆，此处将其保存为了apiclient_test_key.pem，
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1-----BEGIN PRIVATE KEY-----

28-----END PRIVATE KEY-----
```

### 2. 构造签名串

我们希望商户的技术开发人员按照当前文档约定的规则构造签名串。微信支付会使用同样的方式构造签名串。如果商户构造签名串的方式错误，将导致签名验证不通过。下面先说明签名串的具体格式。

签名串一共有五行，每一行为一个参数。结尾以\n（换行符，ASCII编码值为0x0A）结束，包括最后一行。如果参数本身以\n结束，也需要附加一个\n。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1HTTP请求方法\n
2URL\n
3请求时间戳\n
4请求随机串\n
5请求报文主体\n
```

以查询单笔退款（通过商户退款单号）接口为例，

第一步，获取HTTP请求的方法
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1GET
```

第二步，获取请求的绝对URL，查询单笔退款（通过商户退款单号）接口的URL为/v3/refund/domestic/refunds/{out_refund_no}，path里面带了参数out_refund_no，这里需要填上实际的商户退款单号，例如123123123123
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1/v3/refund/domestic/refunds/123123123123
```

第三步，获取发起请求时的系统当前时间戳，即格林威治时间1970年01月01日00时00分00秒(北京时间1970年01月01日08时00分00秒)起至现在的总秒数，作为请求时间戳。微信支付会拒绝处理很久之前发起的请求，请商户保持自身系统的时间准确。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1date +%s 
21554208460
```

第四步，生成一个请求随机串，我们推荐生成随机数算法如下：调用随机数函数生成，将得到的值转换为字符串。这里，我们使用命令行直接生成一个。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1hexdump -n 16 -e '4/4 "%08X" 1 "\n"' /dev/random 
2593BEC0C930BF1AFEB40B4A08C8FB242
```

第五步，获取请求中的请求报文主体（request body）。

对于查询单笔退款（通过商户退款单号）接口的接口来说，请求报文主体是一个空串，只需要附加一个\n即可。

第六步，按照前述规则，构造的请求签名串如下：
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1GET\n 
2/v3/refund/domestic/refunds/123123123123\n 
31554208460\n 
4593BEC0C930BF1AFEB40B4A08C8FB242\n 
5\n
```

### 3. 计算签名值

绝大多数编程语言提供的签名函数支持对签名数据进行签名。强烈建议商户调用该类函数，使用商户API证书私钥对待签名串进行SHA256 with RSA签名，并对签名结果进行Base64编码得到签名值。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1echo -n -e \
2 'GET\n/v3/refund/domestic/refunds/123123123123\n1554208460\n593BEC0C930BF1AFEB40B4A08C8FB242\n\n' \
3 | openssl dgst -sha256 -sign apiclient_test_key.pem \
4 | openssl base64 -A
```

得出的签名值如下，你可以用一些校验工具校验你通过命令行或者你的代码得到的签名值是否和以下值一样，一样则表示计算过程无误，如果不一致则请检查签名的参数是否一致、签名串是否有严格换行、换行转义是否有问题、是否多或者少了/（斜杠）等
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```

```

### 4. 设置HTTP头

微信支付商户API v3要求请求通过HTTP Authorization头来传递签名。Authorization由认证类型和签名信息两个部分组成。

下面我们使用命令行演示如何生成签名。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1Authorization: 认证类型 签名信息
```

具体组成为：

1. 认证类型，目前为WECHATPAY2-SHA256-RSA2048
2. 签名信息发起请求的商户（包括直连商户、服务商或渠道商）的商户号mchid商户API证书序列号serial_no，用于声明所使用的证书，注意这里是商户API证书而不是平台证书，请勿混用，如果不知道如何查看商户API证书序列号，可参考[如何查看商户API证书序列号](https://pay.weixin.qq.com/doc/v3/merchant/4012072428#Q%EF%BC%9A%E5%A6%82%E4%BD%95%E6%9F%A5%E7%9C%8B%E5%95%86%E6%88%B7API%E8%AF%81%E4%B9%A6%E5%BA%8F%E5%88%97%E5%8F%B7%EF%BC%9F)。请求随机串nonce_str，和你上面构造签名串的随机串要保持一致时间戳timestamp，和你上面构造签名串的时间戳要保持一致签名值signature，上面算出来的签名值

| 注意：以上五项签名信息，无顺序要求。 |  | 注意：以上五项签名信息，无顺序要求。 |
| --- | --- | --- |
|  | 注意：以上五项签名信息，无顺序要求。 |  |

Authorization头的示例如下：（注意，示例因为排版可能存在换行，实际数据应在一行）
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1Authorization: WECHATPAY2-SHA256-RSA2048 mchid="1900007291",nonce_str="593BEC0C930BF1AFEB40B4A08C8FB242",signature="",timestamp="1554208460",serial_no="408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB"
```
 
![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/8c6ecfc88a8b8216bbb8938548c3889b.png)

最终我们可以组一个包含了签名的HTTP请求了。
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1curl https://api.mch.weixin.qq.com/v3/refund/domestic/refunds/123123123123 \ 
2-H 'Authorization: WECHATPAY2-SHA256-RSA2048 mchid="1900007291",nonce_str="593BEC0C930BF1AFEB40B4A08C8FB242",signature="",timestamp="1554208460",serial_no="408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB"' -H 'Accept: application/json' \
```

### 5. 演示代码

SDK自带自动构造签名功能，开发者可以查看[SDK](https://pay.weixin.qq.com/doc/v3/merchant/4012076498#%E6%9C%8D%E5%8A%A1%E7%AB%AF-SDK)相关章节，获取对应语言的库。
