...
 你可以点击查看一个API接口说明文档，如[委托营销（查询合作伙伴列表）](https://pay.weixin.qq.com/doc/v3/merchant/4012380536)，如下图，可以看到该接口请求参数部分带有Query参数，因为不同的参数类型构造请求签名略有不同，

为了方便大家理解，我们将使用一对测试的商户API证书公私钥，通过命令行形式调用[委托营销（查询合作伙伴列表）](https://pay.weixin.qq.com/doc/v3/merchant/4012380536)接口，一步一步介绍请求参数里带Query参数（查询参数），如何计算签名。强烈建议你使用测试的商户API证书公私钥，按照本文实际操作一下，如果算出来的签名和文档的一致则说明计算无误。实际业务中请替换为你的真实的参数。
...
构造请求签名需要使用商户API证书，可以参考[申请商户API证书](https://pay.weixin.qq.com/doc/v3/merchant/4012072428)

此处我们已经拿到了商户API证书，并且将对应的商户API证书私钥apiclient_key.pem，保存在了本地
 
商户号：1900007291

商户API证书序列号：408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB

商户API证书私钥：你需要将以下文件保存为pem格式，为了避免大家和自己的真实的公私钥混淆，此处将其保存为了apiclient_test_key.pem
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1-----BEGIN PRIVATE KEY-----
...
1limit=5
2offset=10
3authorized_data=%7B%22business_type%22%3A%22FAVOR_STOCK%22%2C%22stock_id%22%3A%222433405%22%7D
4partner=%7B%22type%22%3A%22APPID%22%2C%22appid%22%3A%22wx4e1916a585d1f4e9%22%2C%22merchant_id%22%3A%222480029552%22%7D
```

（2）拼接你的请求URL,查询参数需要在末尾加'?'和对应的查询字符串，多个字符串之间用&符号链接（请注意以下URL是在一行，因为排版原因可能看起来像换行，实际数据在一行）
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1/v3/marketing/partnerships?limit=5&offset=10&authorized_data=%7B%22business_type%22%3A%22FAVOR_STOCK%22%2C%22stock_id%22%3A%222433405%22%7D&partner=%7B%22type%22%3A%22APPID%22%2C%22appid%22%3A%22wx4e1916a585d1f4e9%22%2C%22merchant_id%22%3A%222480029552%22%7D
```
...
对于[委托营销（查询合作伙伴列表）](https://pay.weixin.qq.com/doc/v3/merchant/4012380536)接口来说，请求报文主体是一个空串，只需要附加一个\n即可。

第六步，按照前述规则，构造的请求签名串如下：
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1GET\n 
2/v3/marketing/partnerships?limit=5&offset=10&authorized_data=%7B%22business_type%22%3A%22FAVOR_STOCK%22%2C%22stock_id%22%3A%222433405%22%7D&partner=%7B%22type%22%3A%22APPID%22%2C%22appid%22%3A%22wx4e1916a585d1f4e9%22%2C%22merchant_id%22%3A%222480029552%22%7D\n 
31554208460\n 
...
2. 签名信息发起请求的商户（包括直连商户、服务商或渠道商）的商户号mchid商户API证书序列号serial_no，用于声明所使用的证书，注意这里是商户API证书而不是平台证书，请勿混用，如果不知道如何查看商户API证书序列号，可参考[如何查看商户API证书序列号](https://pay.weixin.qq.com/doc/v3/merchant/4012072428#Q%EF%BC%9A%E5%A6%82%E4%BD%95%E6%9F%A5%E7%9C%8B%E5%95%86%E6%88%B7API%E8%AF%81%E4%B9%A6%E5%BA%8F%E5%88%97%E5%8F%B7%EF%BC%9F)。请求随机串nonce_str，和你上面构造签名串的随机串要保持一致时间戳timestamp，和你上面构造签名串的时间戳要保持一致签名值signature，上面算出来的签名值

| 注意：以上五项签名信息，无顺序要求。 |  | 注意：以上五项签名信息，无顺序要求。 |
| --- | --- | --- |
|  | 注意：以上五项签名信息，无顺序要求。 |  |
...
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1Authorization: WECHATPAY2-SHA256-RSA2048 mchid="1900007291",nonce_str="593BEC0C930BF1AFEB40B4A08C8FB242",signature="",timestamp="1554208460",serial_no="408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB"
```
...
```
1$curl -X GET \
2https://api.mch.weixin.qq.com/v3/marketing/partnerships?limit=5&offset=10&authorized_data=%7B%22business_type%22%3A%22FAVOR_STOCK%22%2C%22stock_id%22%3A%222433405%22%7D&partner=%7B%22type%22%3A%22APPID%22%2C%22appid%22%3A%22wx4e1916a585d1f4e9%22%2C%22merchant_id%22%3A%222480029552%22%7D \
3-H 'Authorization: WECHATPAY2-SHA256-RSA2048 mchid="1900007291",nonce_str="593BEC0C930BF1AFEB40B4A08C8FB242",signature="",timestamp="1554208460",serial_no="408B07E79B8269FEC3D5D3E6AB8ED163A6A380DB"' \
4-H 'Accept: application/json'
```

### 5. 演示代码

SDK自带自动构造签名功能，开发者可以查看[SDK](https://pay.weixin.qq.com/doc/v3/merchant/4012076498#%E6%9C%8D%E5%8A%A1%E7%AB%AF-SDK)相关章节，获取对应语言的库。
