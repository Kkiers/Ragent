...
在敏感字段加解密的场景，商户需要在发起请求的HTTP头部中通过Wechatpay-Serial添上平台证书（wechatpay.pem）的序列号请注意这里是平台证书序列号（平台证书总是以wechatpay.pem命名），而不是商户API证书序列号（商户API证书总是以apiclient_cert.pem命名），以声明加密所用的平台证书。如何获取平台证书请参考[平台证书简介及使用说明](https://pay.weixin.qq.com/doc/v3/merchant/4012068814)，如何获取证书序列号参考第二部分
...
![](https://gtimg.wechatpay.cn/resource/xres/img/202409/5236a3a065efe1276d29855d7cad26f8_16x16.svg)
```
1$ openssl x509 -in wechatpay.pem -noout -serial
```
