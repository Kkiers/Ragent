...
block_id | string | 父块的`block_id`，表示为其创建一批子块。如果需要对文档树根节点创建子块，可将 `document_id` 填入此处。你可调用[获取文档所有块](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)获取文档中块的 block_id。了解块的父子关系规则，参考[文档概述-基本概念](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)  
...
**数据校验规则**：  
- 最小值：`-1`
client_token | string | 否 | 操作的唯一标识。**UUIDv4** 格式，与接口返回值的 client_token 相对应，用于幂等的进行更新操作。此值为空表示将发起一次新的请求，此值非空表示幂等的进行更新操作  
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
children_id | string\[\] | 是 | 要添加的子块的临时 ID 列表。你需自定义子块的临时 ID，并确保这些 ID 在单次请求中唯一。服务端将生成实际的 Block ID，并在响应体中返回 `block_id_relations` 字段，表示临时 ID 与实际 ID 的映射关系。  
**注意**：仅需写入第一级子块即可，无需写入子块的子块，否则将返回 1770006 错误码。  
**示例值**：["child1", "child2"]  
**数据校验规则**：  
- 长度范围：`1` ～ `1000`
index | int | 否 | 指定在某个块的子块列表中，新创建的子块的放置位置。索引的起始值为 0，表示子块列表的第一个位置；索引的最大值为某个块的子块数量，表示子块列表的最后一个位置。例如：一个块的子块列表中有 5 个子块，那么它们的索引分别为 0、1、2、3、4。如果要在该块的子块列表的中放置一个新创建的子块，并将其放置到第一个位置，那么索引值应为 0；如果要将新创建的子块放置到最后一个位置，那么索引值应为 -1  
**示例值**：0  
**默认值**：`-1`  
**数据校验规则**：  
- 最小值：`-1`
descendants | block\[\] | 是 | 添加的含有父子关系的嵌套块列表，包括父块和子块。了解块的父子关系规则，参考[文档概述-基本概念](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)  
**数据校验规则**：  
- 长度范围：`1` ～ `1000`
block_id | string | 否 | 子块的唯一标识  
**示例值**："doxcnSS4ouQkQEouGSUkTg9NJPe"
children | string\[\] | 否 | 子块的子块 ID 列表  
...
chat_id | string | 是 | 群聊天会话 ID。获取方式参考[群 ID 说明](ssl:ttdoc//uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-id-description)  
...
view_type | int | 否 | 视图类型，卡片视图（默认）或预览视图  
**示例值**：1  
**可选值有**：  
- 1：卡片视图  
- 2：预览视图
grid | grid | 否 | 分栏 Block
column_size | int | 是 | 分栏列数量  
**示例值**：2  
**数据校验规则**：  
- 取值范围：`2` ～ `5`
...
- 15：Undefined  
- 99：Other
url | string | 是 | iframe 目标 url（需要进行 url_encode）  
...
component_id | string | 否 | 文档小组件 ID。该 ID 可通过调用[创建 BlockEntity](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/block-v2/entity/create) 接口，并从响应体中的 block_id 中获取，创建时使用的 `block_type_id` 需要与 `component_type_id` 一致。  
**示例值**："7056882725002051603"
component_type_id | string | 是 | 文档小组件类型，比如问答互动"blk_636a0a6657db8001c8df5488"。该 ID 可在 [开发者后台](https://open.feishu.cn/app) > **应用详情页** > **应用能力** > **云文档小组件** > **BlockTypeID** 获取。  
**示例值**："blk_636a0a6657db8001c8df5488"
record | string | 否 | 文档小组件内容数据，JSON 字符串  
**示例值**：""{}""
sheet | sheet | 否 | 电子表格 Block
row_size | int | 否 | 电子表格行数量  
**示例值**：2  
**数据校验规则**：  
- 取值范围：`1` ～ `9`
column_size | int | 否 | 电子表格列数量  
**示例值**：2  
**数据校验规则**：  
- 取值范围：`1` ～ `9`
...
okr_id | string | 否 | OKR ID，获取需要插入的 OKR ID 可见[获取用户的 OKR 列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list)  
...
objective_id | string | 否 | OKR 中 Objective 的 ID  
**示例值**："7109022409227026460"
kr_ids | string\[\] | 否 | Key Result 的 ID 列表，此值为空时插入当前 Objective 下的所有 Key Result  
**示例值**：["7109022573011894300","7109022546444517404"]
comment_ids | string\[\] | 否 | 评论 id 列表  
**示例值**：[1660030311959965796]
wiki_catalog | wiki_catalog | 否 | Wiki 子目录 Block
wiki_token | string | 否 | 知识库 token  
...
- 3：居右排版  
**默认值**：`1`
width | int | 否 | 宽度，单位 px；不填时自动适应文档宽度；值超出文档最大宽度时，页面渲染为文档最大宽度  
**示例值**：300  
**数据校验规则**：  
- 最小值：`90`
height | int | 否 | 高度，单位 px；不填时自动根据画板内容计算；值超出屏幕两倍高度时，页面渲染为屏幕两倍高度  
**示例值**：300  
**数据校验规则**：  
- 最小值：`90`
...
source_document_id | string | 否 | 源文档的文档 ID  
**示例值**："doxcnSS4ouQkQEouGSUkTgabcef"
source_block_id | string | 否 | 源同步块的 Block ID  
**示例值**："doxcnePuYufKa49ISjhD8Iabcef"
sub_page_list | sub_page_list | 否 | Wiki 新版子目录
wiki_token | string | 是 | 知识库节点 token，仅支持知识库文档创建子页面列表，且需传入当前页面的 wiki token  
...
文档场景中，可参考[文档概述-基本概念](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)了解块的父子关系规则。
