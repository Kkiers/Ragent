...
支持的应用类型 | 自建应用、商店应用
权限要求  
            **订阅该事件所需的权限，开启其中任意一项权限即可订阅** | 订阅文件夹下的文件相关事件(space:document.event:read)
...
event | \- | \-
file_type | string | 文件类型，与文件的 file_token 相匹配  
**示例值**：docx  
**可选值有**：  
- doc：旧版文档  
- docx：新版文档  
...
**数据校验规则**：  
- 长度范围：`22` ～ `27` 字符
folder_token | string | 文件夹 Token。获取方式见[文件夹概述](https://open.feishu.cn/document/ukTMukTMukTM/ugTNzUjL4UzM14CO1MTN/folder-overview)  
**数据校验规则**：  
- 长度范围：`22` ～ `27` 字符
...
subscriber_ids | user_id\[\] | 订阅者的用户 ID 列表  
**数据校验规则**：  
- 长度范围：`0` ～ `100`
