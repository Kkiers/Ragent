...
支持的应用类型 | 自建应用、商店应用
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用** | 创建字段编组(base:field_group:create)
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
field_groups | field_group\[\] | 是 | 要新增字段编组列表  
**数据校验规则**：  
- 长度范围：`1` ～ `300`
id | string | 否 | 字段编组的ID，默认由系统生成新的字段编组ID  
**示例值**："fldPTb0U2y"
name | string | 是 | 字段编组的名称  
**示例值**："用户信息组"  
**数据校验规则**：  
- 长度范围：`1` ～ `100` 字符  
- 正则校验：`^[^[\]]+$`
children | field_group_child\[\] | 是 | 字段编组的成员  
**数据校验规则**：  
- 长度范围：`1` ～ `300`
type | string | 是 | 编组成员类型  
**示例值**："field"  
**可选值有**：  
- field：字段
id | string | 是 | 编组成员ID，必须与type的取值一致（如type为field时，id为字段的ID）；字段ID可以通过调用[获取字段列表]接口获取  
**示例值**："fldPTb0U2y"
description | string | 否 | 字段编组的描述  
**示例值**："用于组织用户信息相关的字段"  
**数据校验规则**：  
- 长度范围：`0` ～ `2000` 字符
