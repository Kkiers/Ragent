...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
member_list | app.role.member_id\[\] | 是 | 协作者列表  
**数据校验规则**：  
- 最大长度：`100`
type | string | 否 | 协作者 ID 的类型  
**示例值**："open_id"  
**可选值有**：  
...
**默认值**：`open_id`
id | string | 是 | 协作者的 ID，需与 type 的类型需一致。获取 ID 方式参考 type 参数描述。  
...
400 | 1254048 | MemberNotFound | 协作者 ID 不存在。请检查：  
