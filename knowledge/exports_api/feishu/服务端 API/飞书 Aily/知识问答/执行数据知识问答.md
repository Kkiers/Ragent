...
**注意事项**：- 此接口以Server-sent Events（SSE）方式更新问答结果，需要注意响应体格式
...
支持的应用类型 | 自建应用
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用** | 执行智能伙伴数据知识问答(aily:knowledge:ask)
...
名称 | 类型 | 描述
--- | --- | ---
app_id | string | 飞书 Aily 平台的AppID，通过飞书 Aily 平台创建应用获取  
...
**数据校验规则**：  
- 长度范围：`1` ～ `65535` 字符
data_asset_ids | string\[\] | 否 | 控制知识问答所依据的数据知识范围，默认值为空，此时不限制数据知识范围  
**示例值**：["asset_aadg2b5os5wjg"]  
**数据校验规则**：  
- 长度范围：`0` ～ `65535`
data_asset_tag_ids | string\[\] | 否 | 控制知识问答所依据的数据知识分类范围，默认值为空，此时不限制数据知识分类范围  
...
msg | string | 错误描述
data | \- | \-
status | string | 响应状态，枚举值  
**可选值有**：  
- processing：当前知识问答正在处理中  
- finished：当前知识问答处理完成
finish_type | string | 结束类型，枚举值  
**可选值有**：  
- qa：执行数据知识问答  
- faq：执行标准问答对
