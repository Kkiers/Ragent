...
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用**  
            开启任一权限即可 | 读取数据表记录(spark:app.table.record:read)  
            修改或删除数据表记录(spark:app.table.record:write)
...
view_name | string | 妙搭视图表名，必须属于 app_id 对应的妙搭应用，可从妙搭应用数据库管理视图列表中获取  
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
page_size | int | 否 | 分页大小，用于限制一次请求所返回的数据条目数。默认10，最大500  
**示例值**：10  
**默认值**：`10`  
**数据校验规则**：  
- 取值范围：`1` ～ `500`
...
select | string | 否 | 返回的列，默认为 *，即返回所有列。  
...
filter | string | 否 | 筛选条件，尊许 PostgREST 语法，详情可查看 https://docs.postgrest.org/en/v13/references/api/tables_views.html#horizontal-filtering  
**示例值**：age=gt.10
order | string | 否 | 排序条件，如果没指定 asc/desc，默认为 asc，null 值可排在最前或最后。  
...
**示例值**：`online`、`dev`  
**默认值**：`online`
user_identifier_type | string | 否 | 此次调用使用的用户 ID 类型，将使用指定的 ID 来标示某个用户在接口入参和出参中的值。  
示例值：`miaoda_user_id`  
可选值：  
