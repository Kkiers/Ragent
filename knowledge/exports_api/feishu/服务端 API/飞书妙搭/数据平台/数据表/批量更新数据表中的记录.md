...
支持的应用类型 | 自建应用
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用** | 修改或删除数据表记录(spark:app.table.record:write)
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
records | string | 是 | 要更新的数据记录列表（JSON数组的字符串形式），单次支持最大长度 500条，每行 record 都必须包含主键如 _id，且不同行要更新的字段需保持一致  
