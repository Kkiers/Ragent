...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
requests | update_block_request\[\] | 否 | 批量更新块。不支持在一次批量更新中，对同一个块进行多次更新。即请求体中的 Block ID 不能重复  
**数据校验规则**：  
- 最大长度：`200`
...
- NoIndent：无缩进  
- OneLevelIndent：一级缩进
sequence | string | 否 | 用于确定有序列表项编号，为具体数值或'auto'  
...
**示例值**："boxbckbfvfcqEg22hAzN8Dabcef"
block_id | string | 否 | Block 唯一标识。你可调用[获取文档所有块](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)获取文档中块的 `block_id`  
