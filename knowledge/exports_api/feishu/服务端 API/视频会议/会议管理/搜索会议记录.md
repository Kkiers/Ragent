...
支持的应用类型 | 自建应用
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用** | 搜索视频会议(vc:meeting.search:read)
...
page_size | int | 否 | 分页大小，**默认15**，最大单页**不超过30**  
**注意：** 最多返回**150条**记录  
**示例值**：10
...
organizer_ids | string\[\] | 否 | 按会议组织者过滤，传入用户 open_id 列表，可通过用户查询接口获取。默认值为空数组，不设置时不过滤该条件。  
**示例值**：["ou_789012"]  
**数据校验规则**：  
- 长度范围：`0` ～ `128`
participant_ids | string\[\] | 否 | 按参会人过滤，传入用户 open_id 列表，可通过用户查询接口获取。默认值为空数组，不设置时不过滤该条件。长度范围：0～128。  
**示例值**：["ou_789012"]  
**数据校验规则**：  
- 长度范围：`0` ～ `128`
open_room_ids | string\[\] | 否 | 按会议室过滤，传入会议室 open_id 列表，可通过会议室查询接口获取。默认值为空数组，不设置时不过滤该条件。长度范围：0～128。  
...
start_time | string | 否 | 时间范围的起始时间，需符合 ISO 8601 标准并携带时区信息。  
**示例值**："2026-03-21T16:15:30+08:00"
end_time | string | 否 | 时间范围的结束时间，需符合 ISO 8601 标准并携带时区信息。  
**示例值**："2026-03-21T16:15:30+08:00"
...
display_info | string | 包含基本信息的卡片，用户搜索关键词命中的文本片段，使用<h></h>标签包裹标注
