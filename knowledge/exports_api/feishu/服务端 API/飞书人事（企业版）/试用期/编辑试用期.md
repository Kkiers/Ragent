...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
employment_id | string | 是 | 试用期人员的雇佣 ID，类型与查询参数 user_id_type取值一致：  
...
probation_start_date | string | 否 | 试用期开始日期，格式："YYYY-MM-DD"，填写时满足以下规则  
- 权限要求：读写试用期开始日期信息(corehr:probation.probation_start_date:write)  
- 「试用期开始日期」和「试用期预计结束日期」需要一同填写。   
- 若该员工存在试用期记录，且同时填写「试用期开始日期」和「试用期预计结束日期」为空串，则删除该试用期记录。   
- 若该员工不存在试用期记录，且填写「试用期开始日期」和「试用期预计结束日期」，则新增试用期记录。   
- 若该员工不存在试用期记录，且未填写「试用期开始日期」和「试用期预计结束日期」，则试用期记录不会创建，其他写入数据不会生效。   
- 入职日期 <= 试用期开始日期 <= 试用期预计结束日期。  
**示例值**："2024-01-01"
probation_expected_end_date | string | 否 | 试用期预计结束日期，格式："YYYY-MM-DD"，填写时满足以下规则   
- 权限要求：读写试用期预计结束日期信息(corehr:probation.probation_expected_end_date:write)  
- 「试用期开始日期」和「试用期预计结束日期」需要一同填写。   
- 若该员工存在试用期记录，且同时填写「试用期开始日期」和「试用期预计结束日期」为空串，则删除该试用期记录。   
- 若该员工不存在试用期记录，且填写「试用期开始日期」和「试用期预计结束日期」，则新增试用期记录。   
- 若该员工不存在试用期记录，且未填写「试用期开始日期」和「试用期预计结束日期」，则试用期记录不会创建，其他写入数据不会生效。   
- 试用期开始日期 <= 试用期预计结束日期  
**示例值**："2025-01-01"
probation_outcome | string | 否 | 试用期结果，填写时满足以下规则   
- 权限要求：写试用期结果字段(corehr:probation.probation_outcome:write)  
- 取值区分大小写。  
- 填写「通过」时，必填「试用期实际结束日期」。  
- 填写「延长」时，「延长后试用期预计结束日期」和「试用期预计延长时长」至少填写一项。  
...
- failed：未通过  
- delayed：延期
actual_probation_end_date | string | 否 | 试用期实际结束日期，格式："YYYY-MM-DD"，填写时满足以下规则  
- 权限要求：读写试用期实际结束日期信息(corehr:probation.actual_probation_end_date:write)  
- 需同时指定试用期结果为「通过」。  
- 试用期开始时间 <= 试用期实际结束日期 <= 试用期预计结束日期。  
**示例值**："2025-01-01"
probation_extend_expected_end_date | string | 否 | 延长后试用期预计结束日期，格式："YYYY-MM-DD"，填写时满足以下规则  
- 权限要求：读写延长后试用期预计结束日期信息(corehr:probation.probation_extend_expected_end_date:write)  
- 需同时指定「试用期结果」为「延期」。  
- 若同时填写「试用期预计延长时长」，需保持两者数据一致。  
- 延长后试用期预计结束日期 > 试用期预计结束日期。  
**示例值**："2025-01-01"
extended_probation_period_duration | int | 否 | 试用期预计延长时长，填写时满足以下规则  
- 权限要求：读写试用期预计延长时长信息(corehr:probation.extended_probation_period_duration:write)  
- 需同时指定「试用期结果」为「延期」。  
- 填写时需要指定「试用期延长时长单位」。  
- 若同时填写「延长后试用期预计结束日期」，需保持两者数据一致。  
- 试用期预计延长时长 > 0。  
**示例值**：1  
**数据校验规则**：  
- 取值范围：`1` ～ `500`
extended_probation_period_unit | string | 否 | 试用期延长时长单位，填写时满足以下规则  
- 权限要求：读写试用期预计延长时长信息(corehr:probation.extended_probation_period_duration:write)  
- 取值区分大小写。  
- 需要和「试用期预计延长时长」一同填写。  
- 需同时指定「试用期结果」为「延期」。  
...
- month：月
notes | string | 否 | 备注  
- 权限要求：读写试用期备注信息(corehr:probation.notes:write)  
**示例值**："试用期表现良好。"
self_review | string | 否 | 员工自评  
- 权限要求：读写员工自评信息(corehr:probation.self_review:write)  
**示例值**："试用期表现良好。"
custom_fields | custom_field_data\[\] | 否 | 自定义字段（当前不支持附件类型字段写入）  
- 权限要求：读写试用期自定义字段信息(corehr:probation.custom_field:write)  
