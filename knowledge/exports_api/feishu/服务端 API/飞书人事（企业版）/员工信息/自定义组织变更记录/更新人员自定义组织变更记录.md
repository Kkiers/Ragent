...
支持的应用类型 | 自建应用、商店应用
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用** | 读写员工的自定义组织信息(corehr:employment.custom_org:write)
...
**示例值**："7293841029445207593"
custom_org_with_rates | custom_org_with_rate\[\] | 是 | 自定义组织列表  
**数据校验规则**：  
- 长度范围：`1` ～ `20`
id | string | 是 | 自定义组织id  
