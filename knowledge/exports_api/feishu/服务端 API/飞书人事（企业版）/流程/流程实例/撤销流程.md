...
名称 | 类型 | 描述
--- | --- | ---
process_id | string | 流程实例 ID。可通过[查询流程实例列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/process/list)接口获取   
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
user_id | string | 否 | 按照查询参数中指定的用户ID类型传递对应的用户ID，默认为Open ID。  
**示例值**："ou_91791271921729102012"
reason | string | 否 | 原因  
**示例值**："原因自定义字符串"  
**数据校验规则**：  
- 最大长度：`500` 字符
system_user | boolean | 否 | true-系统身份操作  
**示例值**：true  
**默认值**：`false`
