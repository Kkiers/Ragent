...
value | string | 是 | 内容，支持中文和英文。  
**示例值**："张三"
post_code | string | 否 | 邮编（该字段已废弃，可忽略）  
**示例值**："100000"
...
value | string | 内容，支持中文和英文
phone_number | string | 号码
registered_office_address | i18n\[\] | 完整注册地址（该字段暂不返回，可忽略）  
...
lang | string | 语言，支持中文和英文。中文用zh-CN；英文用en-US。
value | string | 内容，支持中文和英文
office_address | i18n\[\] | 完整办公地址（该字段暂不返回，可忽略）  
-  调用[【查询单个公司】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/company/get)[【批量查询公司】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/company/list)等接口可以返回完整办公地址
lang | string | 语言，支持中文和英文。中文用zh-CN；英文用en-US。
value | string | 内容，支持中文和英文
