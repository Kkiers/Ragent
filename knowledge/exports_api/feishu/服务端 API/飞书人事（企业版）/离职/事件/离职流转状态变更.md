...
权限要求  
            **订阅该事件所需的权限，开启其中任意一项权限即可订阅**  
            开启任一权限即可 | 订阅离职流转状态变更事件(corehr:offboarding.checklist_status_message:read)  
            获取员工离职信息(corehr:offboarding:read)  
            更新雇佣信息(corehr:employment)  
            获取雇佣信息(corehr:employment:readonly)  
