...
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用**  
            开启任一权限即可 | 更新应用分组(admin:app.category:update)  
            更新应用信息(application:application)
...
**数据校验规则**：  
- 长度范围：`1` ～ `3`
allow_refresh_token | boolean | 否 | 是否允许刷新user_access_token  
**示例值**：false
callback_info | callback_info | 否 | 应用回调配置
callback_type | string | 否 | 回调类型  
**示例值**："webhook"  
**可选值有**：  
- webhook：将回调发送至开发者服务器  
- websocket：使用长连接接收回调
request_url | string | 否 | 回调地址  
**示例值**："https://open.feishu.cn/"
subscribed_callbacks | string\[\] | 否 | 订阅的回调列表  
**示例值**：["url.preview.get"]  
**数据校验规则**：  
- 长度范围：`0` ～ `99999`
