...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
i18ns | app_i18n_info\[\] | 否 | 应用名称描述多语种  
**数据校验规则**：  
- 长度范围：`1` ～ `100`
i18n_key | string | 是 | 必填，国际化语言的 key  
**示例值**："zh_cn"  
**可选值有**：  
- zh_cn：中文  
- en_us：英文  
...
- th_th：泰语  
- ko_kr：韩语
name | string | 否 | i18n_key对应的应用国际化名称  
**示例值**："应用名称"  
**数据校验规则**：  
- 最小长度：`1` 字符
description | string | 否 | 应用国际化描述（副标题）  
**示例值**："应用描述"  
**数据校验规则**：  
- 最小长度：`1` 字符
help_use | string | 否 | 帮助国际化文档链接  
**示例值**："https://www.example.com"
avatar_url | string | 否 | 应用icon图片链接  
**示例值**："https://s3-imfile.feishucdn.com/static-resource/v1/v2_953a8fc1-50bd-4b2e-87e2-b09e47dba23g"  
**数据校验规则**：  
- 最大长度：`512` 字符
homepage_url | string | 否 | 应用管理后台url链接  
