...
employee_id | string | 企业内在职员工的唯一标识，与employee_id_type类型保持一致。注：当 required_fields 字段未指定获取 employee_id 时，系统会返回无意义的 ID；若需获取该字段的值，请在 required_fields 中添加对应的字段名称。  
获取employee_id的方式：  
