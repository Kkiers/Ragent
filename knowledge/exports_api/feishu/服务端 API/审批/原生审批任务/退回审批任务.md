...
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用**  
            ...
            同意、拒绝、退回、加签等原生审批任务操作(approval:task)
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
user_id | string | 是 | 当前审批任务的审批人的用户 ID，ID 类型与查询参数 user_id_type 取值一致。可调用[获取单个审批实例详情](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/get)，从返回结果的 task_list 参数中获取用户 ID 以及任务状态必须为 PENDING。  
**示例值**："893g4c45"
task_id | string | 是 | 当前需要回退的审批任务 ID。可调用[获取单个审批实例详情](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/get)，从返回结果的 task_list 参数中获取任务 ID 以及任务状态必须为 PENDING。  
**示例值**："7026591166355210260"
reason | string | 否 | 退回原因  
**示例值**："申请事项填写不具体，请重新填写"
extra | string | 否 | 扩展字段。  
**注意**：灰度参数，暂未开放使用。  
**示例值**："demo"
task_def_key_list | string\[\] | 是 | 需要退回到的任务 node_key。可调用[获取单个审批实例详情](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/get)，从返回结果的 timeline 参数中获取，且动态类型 type 必须为 PASS。  
