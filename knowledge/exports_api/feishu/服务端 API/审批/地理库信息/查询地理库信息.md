...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
page_size | int | 否 | 分页大小，用于指定一次请求所返回的数据量上限，最大100，默认20  
**示例值**：10
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果  
**示例值**：eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0=
root_district_id | string | 否 | 指定节点进行遍历，仅返回该节点下的数据，默认遍历根节点，返回值内容可参考list_type参数描述  
**示例值**：1816670
list_type | string | 否 | 遍历类型，不同的类型内容会有差异  
**示例值**：sub_level  
**可选值有**：  
- sub_level：遍历指定节点的下一层区域，默认方式  
- leaf_level：遍历指定节点的所有叶子节点，指定该参数时会返回parent_district
locale | string | 否 | 返回指定语言的内容，默认返回英文数据  
**示例值**：zh-CN  
**可选值有**：  
- zh-CN：中文  
- en-US：英文
