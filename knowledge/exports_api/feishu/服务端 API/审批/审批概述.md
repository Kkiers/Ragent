...
使用审批定义的 Approval Code 可以获取审批定义下的所有数据，以及可以订阅该定义下所有审批实例的变更事件，因此需要注意妥善保管审批定义的 Approval Code，避免因 Approval Code 泄露导致的数据安全风险。
...
`POST` /open-apis/approval/v4/instances/cc | 访问审批应用(approval:approval:readonly) | `tenant_access_token` | **✓** | **✓**
