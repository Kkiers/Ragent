...
权限要求  
            **调用该 API 所需的权限。开启其中任意一项权限即可调用**  
            开启任一权限即可 | 创建、更新用户数据迁移(security_and_compliance:user_migration)  
            查询、更新员工的数据驻留地(security_and_compliance:user_migration:multi-geo)
...
名称 | 类型 | 必填 | 描述
--- | --- | --- | ---
user_ids | string\[\] | 是 | 迁移用户 ID 列表，ID类型必须与查询参数user_id_type的取值一致  
**示例值**：["7064408494200487938"]  
**数据校验规则**：  
- 长度范围：`1` ～ `100`
dest_geo | string | 是 | 迁移目标地理位置区域，参数长度2到10字符  
**示例值**："sg"
...
400 | 1781004 | Target geographic location is unavailable | 目标地理位置不可用，请使用 Open API 「获取数据驻留地理位置列表  
」获取可用的地理位置列表
400 | 1781005 | User is already in the target geographic location | 用户已经在目标地理位置，请使用 Open API 「获取数据驻留地理位置列表  
」获取可用的地理位置列表
400 | 1781006 | User is in the process of migration | 用户在迁移中，请使用 Open API 「获取单个用户迁移状态  
」获取当前用户迁移状态
